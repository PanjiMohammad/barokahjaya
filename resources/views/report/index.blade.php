...
@section('content')
                        ...
                        <div class="card">
                            ...
                            <div class="card-body loader-area">
                                ...
                                <div class="table-responsive">
                                    <table id="reportOrderTable" style="width: 100%;" class="table">
                                        <thead>
                                            <tr>
                                                <th style="padding: 10px 10px;">#</th>
                                                <th style="padding: 10px 10px; width: 20%;">Tanggal</th>
                                                <th class="text-capitalize" style="padding: 10px 10px; width: 20%;"><span class="float-left">Invoice</span></th>
                                                <th style="padding: 10px 10px; width: 40%;">Pelanggan</th>
                                                <th style="padding: 10px 10px; width: 20%;">Total</th>
                                            </tr>
                                        </thead>
                                    </table>
                                </div>
    ...
    <!-- important link -->
    <a href="{{ route('report.newDatatables') }}" id="reportsGetData"></a>
@endsection
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateExportLink(start, end) {
                $('#exportPDF').data('href', '/administrator/reports/reportorder/' + start.format('YYYY-MM-DD') + '+' + end.format('YYYY-MM-DD'));
            }
            ...
            $.extend($.fn.dataTable.defaults, {
                ...
                initComplete: function() {
                    ...
                    var $searchInput = $('#reportOrderTable_filter input').addClass('form-control form-control-sm').attr('placeholder', 'Cari Laporan...');
                    $searchInput.parent().addClass('d-flex align-items-center');

                    // Add Bootstrap form-control class to length menu dropdown
                    var $lengthMenu = $('#reportOrderTable_length select').addClass('form-control form-control-sm');
                    ...
                    $('#reportOrderTable_length').addClass('d-flex align-items-center');
                }
            });

            var url = $('#reportsGetData').attr('href');
            var table = $('#reportOrderTable').DataTable({
                ajax: {
                    url: url,
                    data: function(d) {
            ...
            $('#created_at').on('apply.daterangepicker', function(ev, picker) {
                ...
                table.ajax.url('/administrator/reports/order/orderReportDatatables?date=' + startDate + ' - ' + endDate).load();
            ...
            $('#exportPDF').on('click', function(e) {
                ...
                moment.locale('id');
                // Format the start and end dates using moment.locale
                let startDate =moment().startOf('month').locale('id').format('dddd, D MMMM YYYY');
                let endDate = moment().endOf('month').locale('id').format('dddd, D MMMM YYYY');

                Swal.fire({
                    title: 'Konfirmasi',
                    text: 'Apakah Anda yakin ingin mendownload laporan periode ' + startDate + ' - ' + endDate + '?',
                    icon: 'warning',
